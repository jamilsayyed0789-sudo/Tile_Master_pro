...
Write-Host "`n[1/3] Opening Windows Firewall ports 3000 & 8001..." -ForegroundColor Cyan
...
$rule8001 = Get-NetFirewallRule -DisplayName "TileMaster Backend 8001" -ErrorAction SilentlyContinue
if (-not $rule8001) {
    New-NetFirewallRule -DisplayName "TileMaster Backend 8001" -Direction Inbound -Protocol TCP -LocalPort 8001 -Action Allow | Out-Null
    Write-Host "  -> Port 8001 (FastAPI) rule ADDED" -ForegroundColor Green
} else {
    Write-Host "  -> Port 8001 rule already exists" -ForegroundColor Gray
}
...
Start-Process -FilePath "powershell" -ArgumentList "-NoExit", "-Command", "Write-Host 'FastAPI Backend running on http://0.0.0.0:8001' -ForegroundColor Green; Set-Location 'C:\Personal_Work\Tile_box_calculator\backend'; .\venv\Scripts\python.exe main.py" -WindowStyle Normal
